import streamlit as st
import pandas as pd
import datetime
import os
import gspread
import json
from google.oauth2.service_account import Credentials
from PIL import Image
from googleapiclient.discovery import build
from googleapiclient.http import MediaIoBaseUpload
from io import BytesIO

# ========== CONFIG ==========
st.set_page_config(page_title="Order Collection App", layout="centered")
os.makedirs("images", exist_ok=True)
GOOGLE_SHEET_ID = "1hbUt6Qzk_uMeYRX_1bV11JRwFyOpOXU8GutmzKe6TWU"
DISTRIBUTOR_FILE = "distributors.csv"
PRODUCT_FILE = "products.csv"
USERS_FILE = "users.csv"
EMPLOYEE_MAPPING_FILE = "employee_distributor_map.csv"
DRIVE_FOLDER_ID = "1b96TC89lrujL-JG66PZGhteTuQXSRVM0"

# ========== GOOGLE AUTH ==========
@st.cache_resource
def initialize_google_services():
    """Initialize Google Sheets and Drive services with proper error handling"""
    scope = [
        "https://www.googleapis.com/auth/spreadsheets",
        "https://www.googleapis.com/auth/drive.file"
    ]
    
    try:
        # Check if secrets exist
        if "GOOGLE_SERVICE_ACCOUNT" not in st.secrets:
            st.error("❌ GOOGLE_SERVICE_ACCOUNT not found in Streamlit secrets.")
            st.error("Please add your Google Service Account credentials in the Streamlit Cloud app settings.")
            st.stop()
        
        # Get credentials from secrets
        secrets = st.secrets["GOOGLE_SERVICE_ACCOUNT"]
        
        # Handle different secret formats
        if "json_key" in secrets:
            # JSON string format
            creds_dict = json.loads(secrets["json_key"])
        else:
            # Individual fields format
            creds_dict = dict(secrets)
        
        # Validate required fields
        required_fields = ["type", "project_id", "private_key", "client_email"]
        missing_fields = [field for field in required_fields if field not in creds_dict]
        
        if missing_fields:
            st.error(f"❌ Missing required fields in Google Service Account: {', '.join(missing_fields)}")
            st.stop()
        
        # Create credentials
        creds = Credentials.from_service_account_info(creds_dict, scopes=scope)
        
        # Initialize services
        client = gspread.authorize(creds)
        drive_service = build('drive', 'v3', credentials=creds)
        
        # Test Google Sheets connection
        try:
            sheet = client.open_by_key(GOOGLE_SHEET_ID).sheet1
            # Try to read the first row to test access
            sheet.row_values(1)
        except gspread.SpreadsheetNotFound:
            st.error(f"❌ Google Sheet not found with ID: {GOOGLE_SHEET_ID}")
            st.error("Please check the Sheet ID or ensure the service account has access to the sheet.")
            st.stop()
        except Exception as e:
            st.error(f"❌ Failed to access Google Sheet: {str(e)}")
            st.error("Please ensure the service account has 'Editor' access to the Google Sheet.")
            st.stop()
        
        # Test Google Drive connection
        try:
            drive_service.files().get(fileId=DRIVE_FOLDER_ID).execute()
        except Exception as e:
            st.error(f"❌ Failed to access Google Drive folder: {str(e)}")
            st.error("Please ensure the service account has access to the Drive folder.")
            st.stop()
        
        st.success("✅ Google Services initialized successfully!")
        return client, sheet, drive_service
        
    except json.JSONDecodeError:
        st.error("❌ Invalid JSON format in Google Service Account credentials.")
        st.stop()
    except Exception as e:
        st.error(f"❌ Error initializing Google services: {str(e)}")
        st.error("Please check your Google Service Account credentials format.")
        st.stop()

# Initialize Google services
client, sheet, drive_service = initialize_google_services()

# ========== USER LOGIN ==========
ADMIN_CREDENTIALS = {"username": "admin", "password": "admin"}
USERS = {}

# Load users from file
if os.path.exists(USERS_FILE):
    try:
        df_users = pd.read_csv(USERS_FILE)
        for _, row in df_users.iterrows():
            if row["Username"] != ADMIN_CREDENTIALS["username"]:
                USERS[row["Username"]] = {
                    "password": str(row["Password"]),
                    "role": row["Role"]
                }
    except Exception as e:
        st.error(f"Failed to load users file: {e}")
else:
    st.warning("User credentials file not found. Please upload it from admin panel.")

# Initialize session state
if "logged_in" not in st.session_state:
    st.session_state["logged_in"] = False
    st.session_state["username"] = ""
    st.session_state["role"] = ""

# Login form
if not st.session_state["logged_in"]:
    st.title("🔐 Login")
    
    with st.form("login_form"):
        username = st.text_input("Username")
        password = st.text_input("Password", type="password")
        login_btn = st.form_submit_button("Login")
        
        if login_btn:
            if username == ADMIN_CREDENTIALS["username"] and password == ADMIN_CREDENTIALS["password"]:
                st.session_state["logged_in"] = True
                st.session_state["username"] = username
                st.session_state["role"] = "admin"
                st.success("Welcome Admin! Please refresh the page.")
                st.rerun()
            elif username in USERS and USERS[username]["password"] == password:
                st.session_state["logged_in"] = True
                st.session_state["username"] = username
                st.session_state["role"] = USERS[username]["role"]
                st.success(f"Welcome {username} ({st.session_state['role']})! Please refresh the page.")
                st.rerun()
            else:
                st.error("Invalid username or password")
    st.stop()

# Logout button
if st.sidebar.button("🚪 Logout"):
    st.session_state["logged_in"] = False
    st.session_state["username"] = ""
    st.session_state["role"] = ""
    st.rerun()

st.sidebar.write(f"👤 Logged in as: **{st.session_state['username']}** ({st.session_state['role']})")

# ========== ADMIN UPLOAD ==========
if st.session_state["role"] == "admin":
    st.sidebar.header("📤 Admin Panel")

    st.sidebar.markdown("**Upload Distributors Excel** (column: 'Distributor')")
    dist_file = st.sidebar.file_uploader("Distributors File", type=["xlsx", "csv"], key="dist")
    if dist_file:
        try:
            df = pd.read_excel(dist_file) if dist_file.name.endswith("xlsx") else pd.read_csv(dist_file)
            if "Distributor" not in df.columns:
                st.sidebar.error("File must contain a 'Distributor' column")
            else:
                df.to_csv(DISTRIBUTOR_FILE, index=False)
                st.sidebar.success("✅ Distributors updated.")
        except Exception as e:
            st.sidebar.error(f"Error processing distributors file: {e}")

    st.sidebar.markdown("**Upload Products Excel** (column: 'Product')")
    prod_file = st.sidebar.file_uploader("Products File", type=["xlsx", "csv"], key="prod")
    if prod_file:
        try:
            df = pd.read_excel(prod_file) if prod_file.name.endswith("xlsx") else pd.read_csv(prod_file)
            if "Product" not in df.columns:
                st.sidebar.error("File must contain a 'Product' column")
            else:
                df.to_csv(PRODUCT_FILE, index=False)
                st.sidebar.success("✅ Products updated.")
        except Exception as e:
            st.sidebar.error(f"Error processing products file: {e}")

    st.sidebar.markdown("**Upload Users Excel** (columns: 'Username', 'Password', 'Role')")
    users_file = st.sidebar.file_uploader("Users File", type=["xlsx", "csv"], key="users")
    if users_file:
        try:
            df = pd.read_excel(users_file) if users_file.name.endswith("xlsx") else pd.read_csv(users_file)
            required_cols = ["Username", "Password", "Role"]
            missing_cols = [col for col in required_cols if col not in df.columns]
            if missing_cols:
                st.sidebar.error(f"File must contain columns: {', '.join(missing_cols)}")
            else:
                df.to_csv(USERS_FILE, index=False)
                st.sidebar.success("✅ User credentials updated. Please reload the app.")
        except Exception as e:
            st.sidebar.error(f"Error processing users file: {e}")

    st.sidebar.markdown("**Upload Employee Mapping File** (columns: 'Employee', 'Distributor')")
    emp_map_file = st.sidebar.file_uploader("Employee Mapping File", type=["xlsx", "csv"], key="emp_map")
    if emp_map_file:
        try:
            df = pd.read_excel(emp_map_file) if emp_map_file.name.endswith("xlsx") else pd.read_csv(emp_map_file)
            required_cols = ["Employee", "Distributor"]
            missing_cols = [col for col in required_cols if col not in df.columns]
            if missing_cols:
                st.sidebar.error(f"File must contain columns: {', '.join(missing_cols)}")
            else:
                df.to_csv(EMPLOYEE_MAPPING_FILE, index=False)
                st.sidebar.success("✅ Employee-distributor mapping updated.")
                st.sidebar.write("**Mapping Preview:**")
                st.sidebar.dataframe(df)
        except Exception as e:
            st.sidebar.error(f"Error processing employee mapping file: {e}")

# ========== LOAD OPTIONS ==========
employee_names = []
emp_map = pd.DataFrame()

# Load employee mapping
if os.path.exists(EMPLOYEE_MAPPING_FILE):
    try:
        emp_map = pd.read_csv(EMPLOYEE_MAPPING_FILE)
        if st.session_state["role"] == "employee":
            employee_names = emp_map[emp_map["Employee"] == st.session_state["username"]]["Employee"].unique().tolist()
        else:
            employee_names = emp_map["Employee"].unique().tolist()
    except Exception as e:
        st.error(f"Error loading employee mapping: {e}")

# Default options
distributors = ["D1", "D2", "D3"]
products = ["Donut Cake", "Chocochip Muffin", "Banana Muffin", "Brownie"]

# Load distributors from file
if os.path.exists(DISTRIBUTOR_FILE):
    try:
        df_dist = pd.read_csv(DISTRIBUTOR_FILE)
        if "Distributor" in df_dist.columns:
            distributors = df_dist["Distributor"].dropna().unique().tolist()
    except Exception as e:
        st.error(f"Error loading distributors: {e}")

# Load products from file
if os.path.exists(PRODUCT_FILE):
    try:
        df_prod = pd.read_csv(PRODUCT_FILE)
        if "Product" in df_prod.columns:
            products = df_prod["Product"].dropna().unique().tolist()
    except Exception as e:
        st.error(f"Error loading products: {e}")

# ========== HELPER FUNCTIONS ==========
def upload_to_drive(file_path, filename):
    """Upload file to Google Drive and return the web view link"""
    try:
        # Read the file content
        with open(file_path, 'rb') as f:
            file_content = f.read()
        
        media = MediaIoBaseUpload(BytesIO(file_content), mimetype='image/jpeg')
        
        file_metadata = {
            'name': filename, 
            'parents': [DRIVE_FOLDER_ID]
        }
        
        file = drive_service.files().create(
            body=file_metadata, 
            media_body=media, 
            fields='id,webViewLink'
        ).execute()
        
        return file.get('webViewLink', '')
        
    except Exception as e:
        st.error(f"Drive upload error: {str(e)}")
        return ""

def clear_form_data():
    """Clear form data from session state"""
    keys_to_clear = [
        'employee_name', 'shop_name_input', 'margin_input', 'beat_area_input', 
        'distributor_select', 'remarks_input', 'num_visits', 'last_visited'
    ]
    
    for key in keys_to_clear:
        if key in st.session_state:
            del st.session_state[key]
    
    # Clear product quantities
    for product in products:
        for prefix in ["product_", "stock_"]:
            key = f"{prefix}{product.replace(' ', '_')}"
            if key in st.session_state:
                del st.session_state[key]

# ========== EMPLOYEE FORM ==========
if st.session_state["role"] == "employee":
    st.title("📋 Order Collection Form")
    st.markdown("You can upload or take a photo using your phone camera.")

    with st.form("order_form"):
        st.subheader("🧑 Employee Info")
        
        if not employee_names:
            st.error("No employee mapping found. Please contact admin to set up employee-distributor mapping.")
            st.stop()
        
        name = st.selectbox("Your Name", employee_names, key="employee_name")

        # Filter distributors based on employee
        filtered_distributors = distributors
        if not emp_map.empty and name:
            mapped_distributors = emp_map[emp_map["Employee"] == name]["Distributor"].dropna().unique().tolist()
            if mapped_distributors:
                filtered_distributors = mapped_distributors

        st.subheader("🏪 Shop Details")
        order_date = st.date_input("📅 Order Date", value=datetime.date.today())
        last_visited_date = st.date_input("📅 Last Visited Date")
        num_visits = st.number_input("🔄 Number of Visits", min_value=1, step=1)
        photo = st.file_uploader("📸 Upload / Take Shop Photo", type=["jpg", "jpeg", "png"])
        distributor = st.selectbox("Select Distributor", filtered_distributors, key="distributor_select")
        shop_name = st.text_input("Shop Name", key="shop_name_input")
        margin = st.number_input("Margin (%)", min_value=0.0, max_value=100.0, value=20.0, step=0.1, key="margin_input")
        beat_area = st.text_input("Beat Area", key="beat_area_input")

        st.markdown("### 🧾 Enter Product Quantity and Stock on Hand")
        
        # Create header
        header_cols = st.columns([2, 1, 1])
        header_cols[0].markdown("**Product**")
        header_cols[1].markdown("**Quantity**")
        header_cols[2].markdown("**Stock on Hand**")

        order_details = {}
        stock_on_hand = {}

        # Create product input rows
        for product in products:
            cols = st.columns([2, 1, 1])
            cols[0].write(product)

            qty_key = f"product_{product.replace(' ', '_')}"
            soh_key = f"stock_{product.replace(' ', '_')}"

            qty = cols[1].number_input("", min_value=0, step=1, key=qty_key, label_visibility="collapsed")
            soh = cols[2].number_input("", min_value=0, step=1, key=soh_key, label_visibility="collapsed")

            if qty > 0:
                order_details[product] = qty
                stock_on_hand[product] = soh

        st.subheader("📝 Additional Information")
        remarks = st.text_area("Remarks (Optional)", key="remarks_input")

        submitted = st.form_submit_button("✅ Submit Order")

    # Handle form submission
    if submitted:
        # Validation
        if not name or not shop_name or not photo:
            st.error("❌ Please complete all required fields and upload a photo.")
        elif sum(order_details.values()) == 0 and all(v == 0 for v in stock_on_hand.values()) and not remarks.strip():
            st.error("❌ If no quantity or stock is entered, remarks must be filled.")
        else:
            try:
                # Generate timestamp and filename
                timestamp = datetime.datetime.now().strftime("%Y%m%d_%H%M%S")
                photo_filename = f"{shop_name.replace(' ', '_')}_{timestamp}.jpg"
                photo_path = os.path.join("images", photo_filename)
                
                # Save photo locally
                with open(photo_path, "wb") as f:
                    f.write(photo.read())

                # Upload to Google Drive
                drive_url = upload_to_drive(photo_path, photo_filename)

                # Prepare data for Google Sheets
                if not order_details:
                    # No products ordered, just submit shop visit info
                    row_data = [
                        timestamp, str(order_date), name, distributor, shop_name,
                        margin, beat_area, "", "", "",
                        photo_path, drive_url, remarks, str(last_visited_date), num_visits
                    ]
                    sheet.append_row(row_data)
                else:
                    # Submit each product as a separate row
                    for product, qty in order_details.items():
                        row_data = [
                            timestamp, str(order_date), name, distributor, shop_name,
                            margin, beat_area, product, qty, stock_on_hand.get(product, 0),
                            photo_path, drive_url, remarks, str(last_visited_date), num_visits
                        ]
                        sheet.append_row(row_data)

                # Success messages
                st.success("✅ Order submitted and saved to Google Sheet!")
                if drive_url:
                    st.success(f"📸 Photo uploaded to Google Drive: [View Photo]({drive_url})")
                
                # Display uploaded photo
                st.image(Image.open(photo_path), caption="Uploaded Shop Photo", use_column_width=True)
                
                # Clear form and show info
                st.info("ℹ️ Form data cleared. You can now enter data for another shop.")
                clear_form_data()
                
            except Exception as e:
                st.error(f"❌ Error submitting order: {str(e)}")
                st.error("Please try again or contact support.")

# ========== ADMIN DASHBOARD ==========
elif st.session_state["role"] == "admin":
    st.title("👨‍💼 Admin Dashboard")
    
    # Display recent submissions
    try:
        # Get all data from the sheet
        all_data = sheet.get_all_records()
        if all_data:
            df = pd.DataFrame(all_data)
            st.subheader("📊 Recent Submissions")
            st.dataframe(df.tail(10))  # Show last 10 entries
            
            # Download link for full data
            csv = df.to_csv(index=False)
            st.download_button(
                label="📥 Download Full Data as CSV",
                data=csv,
                file_name=f"order_data_{datetime.date.today()}.csv",
                mime="text/csv"
            )
        else:
            st.info("No data found in the Google Sheet.")
    except Exception as e:
        st.error(f"Error loading data from Google Sheet: {str(e)}")

else:
    st.error("Invalid role. Please contact administrator.")
    st.stop()